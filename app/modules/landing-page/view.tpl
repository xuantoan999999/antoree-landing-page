<div id="mod-landing-page">
	<!-- <div id="popup-ad">
		<button id="popup" class="hidden">Lorem ipsum dolor sit amet</button>
		<div id="popup-ads" class="bzPopup bzPopupAnimation mfp-hide">
			<a href="http://antoree.com/vi/blog/uu-dai-2000000d-khoa-hoc-tieng-anh-cho-nguoi-di-lam-dang-ki-truoc-ngay-0105-343">
				<img src="images/New-Layout/ads15.jpg" alt="" class="img-responsive">
			</a>
		</div>
	</div> -->
	<div id="banner">
		<div class="container-fluid">
			<h1>
				HỌC TIẾNG ANH TRỰC TUYẾN<br>
				1 KÈM 1 VỚI GIA SƯ BẢN NGỮ
			</h1>

			<p class="with">
				cùng cộng đồng hơn 500.000 ngàn người
			</p>
			<p class="facebook">
				<img src="images/New-Layout/facebook-like.png" alt="">
			</p>
			<a href="" class="new-button">
				ĐĂNG KÝ HỌC THỬ MIỄN PHÍ
			</a>
			<a href="" class="popup-youtube" id="youtube-1">
				<amp-img src='images/New-Layout/play-youtube-popup.png' width="80" height="80"></amp-img>
			</a>
			<div id="popup-banner" class="bzPopup bzPopupAnimation mfp-hide">
				<amp-youtube data-videoid="qkt8mJol_KY" layout="responsive" width="480" height="270"></amp-youtube>
			</div>

		</div>
	</div>

	<div id="learn-eng">
		<div class="container-fluid">
			<div id="learn-eng">
				<h2 class="h2Title">HỌC TIẾNG ANH TRÊN ANTOREE</h2>
				<div class="list">
					<div class="learn">
						<div class="head-learn">
							<div class="icon">
								<amp-img src='images/New-Layout/b-1.png' width="48" height="70"></amp-img>
							</div>
							<amp-img src='images/New-Layout/arrow-circle.png' media="(min-width: 1023px)" width="26" height="26"></amp-img>
							<div class="line-long near right"></div>
						</div>
						<p class="text-center">Đăng ký trên website và để lại thông tin số điện thoại cùng email</p>
					</div>
					<div class="learn">
						<div class="head-learn">
							<div class="icon">
								<amp-img src='images/New-Layout/b-2.png' width="75" height="70"></amp-img>
							</div>
							<amp-img src='images/New-Layout/arrow-circle.png' media="(min-width: 1023px)" width="26" height="26"></amp-img>
							<div class="line-long"></div>
						</div>
						<p class="text-center">Trong 05 ngày Chuyên viên tư vấn tuyển sinh gọi lại hỗ trợ thông tin</p>
					</div>
					<div class="learn">
						<div class="head-learn">
							<div class="icon">
								<amp-img src='images/New-Layout/b-3.png' width="65" height="70"></amp-img>
							</div>
							<amp-img src='images/New-Layout/arrow-circle.png' media="(min-width: 1023px)" width="26" height="26"></amp-img>
							<div class="line-long"></div>
						</div>
						<p class="text-center">Làm bài kiểm tra trình độ tiếng Anh theo chuẩn quốc tế</p>
					</div>

					<div class="learn col">
						<div class="head-learn">
							<div class="icon">
								<amp-img src='images/New-Layout/b-4.png' width="72" height="70"></amp-img>
							</div>
							<amp-img src='images/New-Layout/arrow-circle.png' media="(min-width: 1023px)" width="26" height="26"></amp-img>
							<div class="line-long"></div>
						</div>
						<p class="text-center">Hoàn thành thủ tục nhập học và đóng học phí</p>
					</div>
					<div class="learn col">
						<div class="head-learn">
							<div class="icon">
								<amp-img src='images/New-Layout/b-5.png' width="85.75" height="70"></amp-img>
							</div>
							<amp-img src='images/New-Layout/arrow-circle.png' media="(min-width: 1023px)" width="26" height="26"></amp-img>
							<div class="line-long near left"></div>
						</div>
						<p class="text-center">Bắt đầu ngay với lộ trình học của riêng bạn</p>
					</div>
					<div class="clearfix"></div>
				</div>
			</div>
		</div>
	</div>
	<div id="register">
		<div class="content">
			<div class="container-fluid">
				<a href="" target="_blank" class="new-button" id="btDangKyODuoi">ĐĂNG KÝ HỌC NGAY</a>
			</div>
		</div>
	</div>

	<div id="information">
		<div class="container-fluid">

			<div class="row information-list">
				<div class="col-sm-4">
					<div class="information-image float-left">
						<amp-img src="images/New-Layout/infor-1.png" width="97" height="97"></amp-img>
					</div>
					<div class="information-text text-left">
						<p class="head">200+</p>
						<p class="infor-text">lớp học diễn ra hàng ngày</p>
					</div>
					<div class="clearfix"></div>
				</div>
				<div class="col-sm-4">
					<div class="information-image float-left">
						<amp-img src="images/New-Layout/infor-2.png" width="100" height="100"></amp-img>
					</div>
					<div class="information-text text-left">
						<p class="head">1000+</p>
						<p class="infor-text">gia sư đến từ nhiều quốc gia</p>
					</div>
					<div class="clearfix"></div>
				</div>
				<div class="col-sm-4">
					<div class="information-image float-left">
						<amp-img src="images/New-Layout/infor-3.png" width="95" height="95"></amp-img>
					</div>
					<div class="information-text text-left">
						<p class="head">2000+</p>
						<p class="infor-text">học viên từ 4 tuổi đến 45 tuổi</p>
					</div>
					<div class="clearfix"></div>
				</div>
				<div class="clearfix"></div>
			</div>
		</div>
	</div>

	<div id="lesson-antoree">
		<h2 class="h2Title">LỚP HỌC TRÊN ANTOREE</h2>

		<div class="video">
			<amp-youtube data-videoid="o8kvtzBT940" layout="responsive" width="640" height="360"></amp-youtube>
		</div>
		<div class="button">
			<a href="" class="new-button trial-learn">ĐĂNG KÝ HỌC THỬ</a>
		</div>
	</div>

	<div id="best-teacher">
		<h2 class="h2Title">GIÁO VIÊN TIÊU BIỂU</h2>
		<div class="content-best-teacher">
			<div class="teacher-grid-list">
				<div class="teacher-list">
					<div class="teacher gridder-list" data-griddercontent="#teacher-1">
						<div class="teacher-content">
							<amp-img src="images/New-Layout/teacher-best-1.jpg" width="95" height="95" layout="responsive"></amp-img>
							<div class="detail-teacher">
								<div class="info-teacher">
									<div class="teacher-name">
										<span>ZINC AMIDE</span>
									</div>
									<div class="teacher-place">
										<span>Pakistan</span>
									</div>
									<div class="teacher-score">
										<ul class="list-inline">
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star_half-2.png"></li>
										</ul>
										<span class="score-text">(4,71)</span>
									</div>
								</div>
							</div>
							<img class="image-arrow-teacher" src="images/New-Layout/teacher-best-arrow.png">
						</div>
						<div id="teacher-1" class="gridder-content hidden">
							<div class="header">
								<span class="name">ZINC AMIDE</span>
								<span class="place"> - Pakistan</span>
								<div class="score-content">
									<ul class="list-inline">
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star_half-2.png"></li>
									</ul>
								</div>
								<span class="score-text">(4,78)</span>
							</div>
							<div class="content">
								<p>Nhà nghiên cứu khoa học.</p>
								<p>IELTS 8.0</p>
								<p>Học tiếng Anh từ khi 3 tuổi và sử dụng tiếng Anh thường xuyên trong công việc và cuộc sống. Thầy Zinc có kinh nghiệm dạy giao tiếp tiếng Anh hàng ngày, luyện thi IELTS (speaking & writing), luyện viết luận và các kĩ năng giao tiếp nâng cao như phỏng vấn xin việc, kĩ năng tranh biện. Ngoài ra thầy cũng tham gia vào việc xây dựng và phát triển đội ngũ gia sư cho Antoree.</p>
							</div>
							<div class="button-register-home">
								<a href="">ĐĂNG KÝ HỌC VỚI GIA SƯ NÀY</a>
							</div>
						</div>
					</div>
					<div class="teacher gridder-list" data-griddercontent="#teacher-2">
						<div class="teacher-content">
							<amp-img src="images/New-Layout/teacher-best-2.jpg" width="95" height="95" layout="responsive"></amp-img>
							<div class="detail-teacher">
								<div class="info-teacher">
									<div class="teacher-name">
										<span>MIA</span>
									</div>
									<div class="teacher-place">
										<span>Philippines</span>
									</div>
									<div class="teacher-score">
										<ul class="list-inline">
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star_half-2.png"></li>
										</ul>
										<span class="score-text">(4,78)</span>
									</div>
								</div>
							</div>
							<img class="image-arrow-teacher" src="images/New-Layout/teacher-best-arrow.png">
						</div>
						<div id="teacher-2" class="gridder-content hidden">
							<div class="header">
								<span class="name">MIA</span>
								<span class="place"> - Philippines</span>
								<div class="score-content">
									<ul class="list-inline">
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star_half-2.png"></li>
									</ul>
									<span class="score-text">(4,78)</span>
								</div>
							</div>
							<div class="content">
								<p>Cử nhân quản trị kinh doanh.</p>
								<p>Cô Mia luôn được các học viên yêu mến và đánh giá cao vì sự nhiệt tình, thân thiện và phương pháp giảng dạy lôi cuốn. Cô có rất nhiều học viên ở các lứa tuổi, đặc biệt là trẻ em. Cô Mia tham gia giảng dạy về phát âm, ngữ pháp, nâng cao vốn từ vựng trong giao tiếp hàng ngày, giao tiếp trong công việc. Ngoài ra, cô cũng rất sẵn lòng hỗ trợ các học viên nâng cao kĩ năng viết và hiểu biết về marketing</p>
							</div>
							<div class="button-register-home">
								<a href="">ĐĂNG KÝ HỌC VỚI GIA SƯ NÀY</a>
							</div>
						</div>
					</div>
					<div class="teacher gridder-list" data-griddercontent="#teacher-3">
						<div class="teacher-content">
							<amp-img src="images/New-Layout/teacher-best-3.jpg" width="95" height="95" layout="responsive"></amp-img>
							<div class="detail-teacher">
								<div class="info-teacher">
									<div class="teacher-name">
										<span>MATT WILKIE</span>
									</div>
									<div class="teacher-place">
										<span>Pakistan</span>
									</div>
									<div class="teacher-score">
										<ul class="list-inline">
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star_half-2.png"></li>
										</ul>
										<span class="score-text">(4,8)</span>
									</div>
								</div>
							</div>
							<img class="image-arrow-teacher" src="images/New-Layout/teacher-best-arrow.png">
						</div>
						<div id="teacher-3" class="gridder-content hidden">
							<div class="header">
								<span class="name">MATT WILKIE</span>
								<span class="place"> - Pakistan</span>
								<div class="score-content">
									<ul class="list-inline">
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star_half-2.png"></li>
									</ul>
									<span class="score-text">(4,78)</span>
								</div>
							</div>
							<div class="content">
								<p>Giám đốc Kỹ thuật</p>
								<p>Nhiều năm kinh nghiệm dạy tiếng Anh và nghiệp vụ cho các kĩ sư, học viên đến từ Oman, Dubai, Qatar, Pakistan, Ấn Độ, Philippines và Anh. Thầy Matt có thể dạy phát âm tiếng Anh, tiếng Anh giao tiếp trong công việc và cuộc sống. Thầy khuyến khích học viên tích cực sử dụng tiếng Anh ngoài môi trường lớp học, tập trung vào sở thích và thế mạnh để giúp học viên giao tiếp trôi chảy và tự nhiên</p>
							</div>
							<div class="button-register-home">
								<a href="">ĐĂNG KÝ HỌC VỚI GIA SƯ NÀY</a>
							</div>
						</div>
					</div>
					<div class="teacher gridder-list" data-griddercontent="#teacher-4">
						<div class="teacher-content">
							<amp-img src="images/New-Layout/teacher-best-4.jpg" width="95" height="95" layout="responsive"></amp-img>
							<div class="detail-teacher">
								<div class="info-teacher">
									<div class="teacher-name">
										<span>REBECA</span>
									</div>
									<div class="teacher-place">
										<span>Argentina</span>
									</div>
									<div class="teacher-score">
										<ul class="list-inline">
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
										</ul>
										<span class="score-text">(5)</span>
									</div>
								</div>
							</div>
							<img class="image-arrow-teacher" src="images/New-Layout/teacher-best-arrow.png">
						</div>
						<div id="teacher-4" class="gridder-content hidden">
							<div class="header">
								<span class="name">REBECA</span>
								<span class="place"> - Argentina</span>
								<div class="score-content">
									<ul class="list-inline">
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star_half-2.png"></li>
									</ul>
									<span class="score-text">(4,78)</span>
								</div>
							</div>
							<div class="content">
								<p>Giáo viên tiếng Anh</p>
								<p>10 năm kinh nghiệm giảng dạy và làm việc tại các trung tâm tiếng Anh, tập trung vào các mảng tiếng Anh giao tiếp, ngữ pháp, luyện thi chứng chỉ Cambridge ( (KET, PET, FCE, CAE, CPE, IELTS và TOEFL) Cô Rebecca có nghiệp vụ sư phạm bài bản và phương pháp giảng dạy phong phú, ngoài ra cô còn có thể nói được tiếng Pháp, Bồ Đào Nha và Hy Lạp</p>
							</div>
							<div class="button-register-home">
								<a href="">ĐĂNG KÝ HỌC VỚI GIA SƯ NÀY</a>
							</div>
						</div>
					</div>
					<div class="teacher gridder-list" data-griddercontent="#teacher-5">
						<div class="teacher-content">
							<amp-img src="images/New-Layout/teacher-best-5.jpg" width="95" height="95" layout="responsive"></amp-img>
							<div class="detail-teacher">
								<div class="info-teacher">
									<div class="teacher-name">
										<span>JOSEPH MASON</span>
									</div>
									<div class="teacher-place">
										<span>United States</span>
									</div>
									<div class="teacher-score">
										<ul class="list-inline">
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
										</ul>
										<span class="score-text">(5)</span>
									</div>
								</div>
							</div>
							<img class="image-arrow-teacher" src="images/New-Layout/teacher-best-arrow.png">
						</div>
						<div id="teacher-5" class="gridder-content hidden">
							<div class="header">
								<span class="name">JOSEPH MASON</span>
								<span class="place"> - United States</span>
								<div class="score-content">
									<ul class="list-inline">
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star_half-2.png"></li>
									</ul>
									<span class="score-text">(4,78)</span>
								</div>
							</div>
							<div class="content">
								<p>Giáo viên tiếng Anh</p>
								<p>Bằng thạc sĩ giảng dạy ngôn ngữ Anh và chứng chỉ dạy tiếng Anh toàn cầu</p>
								<p>16 năm kinh nghiệm giảng dạy tiếng Anh với 14.000 học sinh trên thế giới. Thầy Joseph có thể dạy IELTS, TOEFL và SLEP, tiếng Anh thương mại và giao tiếp hàng ngày, chỉnh sửa và hướng dẫn cách viết các bài luận cho hâp dẫn. Thầy cũng có thể giúp học sinh chuẩn bị cho phỏng vấn visa đi Mỹ cũng như tư vấn về thủ tục cho học viên muốn học tập tại Mỹ.</p>
							</div>
							<div class="button-register-home">
								<a href="">ĐĂNG KÝ HỌC VỚI GIA SƯ NÀY</a>
							</div>
						</div>
					</div>
					<div class="teacher gridder-list" data-griddercontent="#teacher-6">
						<div class="teacher-content">
							<amp-img src="images/New-Layout/teacher-best-6.jpg" width="95" height="95" layout="responsive"></amp-img>
							<div class="detail-teacher">
								<div class="info-teacher">
									<div class="teacher-name">
										<span>KIT KAT</span>
									</div>
									<div class="teacher-place">
										<span>Việt Nam</span>
									</div>
									<div class="teacher-score">
										<ul class="list-inline">
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star_half-2.png"></li>
										</ul>
										<span class="score-text">(4,91)</span>
									</div>
								</div>
							</div>
							<img class="image-arrow-teacher" src="images/New-Layout/teacher-best-arrow.png">
						</div>
						<div id="teacher-6" class="gridder-content hidden">
							<div class="header">
								<span class="name">KIT KAT</span>
								<span class="place"> - Việt Nam</span>
								<div class="score-content">
									<ul class="list-inline">
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star_half-2.png"></li>
									</ul>
									<span class="score-text">(4,78)</span>
								</div>
							</div>
							<div class="content">
								<p>Cử nhân đại học Khoa học xã hội và Nhân văn thành phố Hồ Chí Minh, chuyên ngành ngôn ngữ và văn học Anh. 4 năm kinh nghiệm dạy tiếng Anh, 2 năm dạy tiếng Anh online. Cô Kit Kat dạy ngữ pháp, phát âm và tiếng Anh giao tiếp hàng ngày hoặc trong công việc. Cô luôn cố gắng tìm kiếm phương pháp giảng dạy mới và hiệu quả, chú trọng vào sửa sai và luyện tập liên tục để học viên không còn cảm thấy gặp trở ngại khi học tiếng Anh. Từ đó học viên có thể yêu thích học tiếng Anh và không coi đó là một gánh nặng.</p>
							</div>
							<div class="button-register-home">
								<a href="">ĐĂNG KÝ HỌC VỚI GIA SƯ NÀY</a>
							</div>
						</div>
					</div>
					<div class="teacher gridder-list" data-griddercontent="#teacher-7">
						<div class="teacher-content">
							<amp-img src="images/New-Layout/teacher-best-7.jpg" width="95" height="95" layout="responsive"></amp-img>
							<div class="detail-teacher">
								<div class="info-teacher">
									<div class="teacher-name">
										<span>DORA</span>
									</div>
									<div class="teacher-place">
										<span>Argentina</span>
									</div>
									<div class="teacher-score">
										<ul class="list-inline">
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star_half-2.png"></li>
										</ul>
										<span class="score-text">(4,91)</span>
									</div>
								</div>
							</div>
							<img class="image-arrow-teacher" src="images/New-Layout/teacher-best-arrow.png">
						</div>
						<div id="teacher-7" class="gridder-content hidden">
							<div class="header">
								<span class="name">DORA</span>
								<span class="place"> - Argentina</span>
								<div class="score-content">
									<ul class="list-inline">
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star_half-2.png"></li>
									</ul>
									<span class="score-text">(4,78)</span>
								</div>
							</div>
							<div class="content">
								<p>Giáo viên tiếng Anh</p>
								<p>5 năm kinh nghiệm giảng dạy tiếng Anh online và offline với học viên thuộc nhiều lứa tuổi từ trẻ em cho tới người đi làm tại Việt Nam, Trung Quốc, Hàn Quốc. Phương pháp giảng dạy của cô Dora rất đa dạng phù hợp với từng học viên, giúp nâng cao kĩ năng phát âm, sử dụng ngữ pháp và giao tiếp hàng ngày tự tin.</p>
							</div>
							<div class="button-register-home">
								<a href="">ĐĂNG KÝ HỌC VỚI GIA SƯ NÀY</a>
							</div>
						</div>
					</div>
					<div class="teacher gridder-list" data-griddercontent="#teacher-8">
						<div class="teacher-content">
							<amp-img src="images/New-Layout/teacher-best-8.jpg" width="95" height="95" layout="responsive"></amp-img>
							<div class="detail-teacher">
								<div class="info-teacher">
									<div class="teacher-name">
										<span>ELSA</span>
									</div>
									<div class="teacher-place">
										<span>Philippines</span>
									</div>
									<div class="teacher-score">
										<ul class="list-inline">
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star_half-2.png"></li>
										</ul>
										<span class="score-text">(4,7)</span>
									</div>
								</div>
							</div>
							<img class="image-arrow-teacher" src="images/New-Layout/teacher-best-arrow.png">
						</div>
						<div id="teacher-8" class="gridder-content hidden">
							<div class="header">
								<span class="name">ELSA</span>
								<span class="place"> - Philippines</span>
								<div class="score-content">
									<ul class="list-inline">
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star_half-2.png"></li>
									</ul>
									<span class="score-text">(4,78)</span>
								</div>
							</div>
							<div class="content">
								<p>Cử nhân Quản trị kinh doanh bằng tiếng Anh</p>
								<p>Chứng chỉ TESOL cho giáo viên tiếng Anh</p>
								<p>14 năm kinh nghiệm giảng dạy tiếng Anh cho các học viên đến từ châu Á, châu Âu, châu Phi và Mỹ La tinh.</p>
								<p>Cô Elsa có thế mạnh ở các mảng ngữ pháp, phát âm, giao tiếp hàng ngày và giao tiếp thương mại bằng tiếng Anh</p>
								<p>Phương pháp giảng dạy: sử dụng hình ảnh từ sách báo tạp chí để giúp học sinh dễ hình dung và ghi nhớ từ vựng, luyện phát âm qua các bài hát</p>
							</div>
							<div class="button-register-home">
								<a href="">ĐĂNG KÝ HỌC VỚI GIA SƯ NÀY</a>
							</div>
						</div>
					</div>
					<div class="teacher gridder-list" data-griddercontent="#teacher-9">
						<div class="teacher-content">
							<amp-img src="images/New-Layout/teacher-best-9.jpg" width="95" height="95" layout="responsive"></amp-img>
							<div class="detail-teacher">
								<div class="info-teacher">
									<div class="teacher-name">
										<span>KENNETH WYATT</span>
									</div>
									<div class="teacher-place">
										<span>Australia</span>
									</div>
									<div class="teacher-score">
										<ul class="list-inline">
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
										</ul>
										<span class="score-text">(5)</span>
									</div>
								</div>
							</div>
							<img class="image-arrow-teacher" src="images/New-Layout/teacher-best-arrow.png">
						</div>
						<div id="teacher-9" class="gridder-content hidden">
							<div class="header">
								<span class="name">KENNETH WYATT</span>
								<span class="place"> - Australia</span>
								<div class="score-content">
									<ul class="list-inline">
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star_half-2.png"></li>
									</ul>
									<span class="score-text">(4,78)</span>
								</div>
							</div>
							<div class="content">
								<p>Thạc sỹ ngôn ngữ học tại đại học Melbourne (Top 8 trường đại học của Úc).</p>
								<p>Ngoài tiếng Anh còn biết tiếng Việt và tiếng Nhật. Có kiến thức chuyên sâu trong các lĩnh vực: Nghệ thật, Công nghệ, Lịch sử, Triết học, Kinh tế. Thầy Ken có tên tiếng Việt thân mật là Kiên, thầy rất vui tính và luôn tạo không khí thoải mái trong các giờ dạy học. Với vốn hiểu biết rộng và kinh nghiệm giảng dạy tiếng Anh, thầy Kenn có thể giúp học viên tiến bộ trong các mảng phát âm, từ vựng, kĩ năng nghe, nói và viết tiếng Anh trôi chảy.</p>
							</div>
							<div class="button-register-home">
								<a href="">ĐĂNG KÝ HỌC VỚI GIA SƯ NÀY</a>
							</div>
						</div>
					</div>
					<div class="teacher gridder-list" data-griddercontent="#teacher-10">
						<div class="teacher-content">
							<amp-img src="images/New-Layout/teacher-best-10.jpg" width="95" height="95" layout="responsive"></amp-img>
							<div class="detail-teacher">
								<div class="info-teacher">
									<div class="teacher-name">
										<span>CHAU TRAN</span>
									</div>
									<div class="teacher-place">
										<span>Việt Nam</span>
									</div>
									<div class="teacher-score">
										<ul class="list-inline">
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star-2.png"></li>
											<li><img src="images/ic_star_half-2.png"></li>
											<li class="review-text">
											</li>
										</ul>
										<span class="score-text">(4,73)</span>
									</div>
								</div>
							</div>
							<img class="image-arrow-teacher" src="images/New-Layout/teacher-best-arrow.png">
						</div>
						<div id="teacher-10" class="gridder-content hidden">
							<div class="header">
								<span class="name">CHAU TRAN</span>
								<span class="place"> - Việt Nam</span>
								<div class="score-content">
									<ul class="list-inline">
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star-2.png"></li>
										<li><img src="images/ic_star_half-2.png"></li>
									</ul>
									<span class="score-text">(4,78)</span>
								</div>
							</div>
							<div class="content">
								<p>Cử nhân quản trị du lịch tại đại học Houston, Texas, Mỹ.</p>
								<p>Cô Châu có nhiều năm sinh sống, học tập và làm việc tại Mỹ với phát âm chuẩn như người bản xứ. Cô tham gia giảng dạy về phát âm, ngữ pháp, tiếng Anh Giao tiếp Thương mại, tiếng Anh chuyên ngành cũng như luyện thi IELTS, TOEIC và TOEFL. Với kinh nghiệm và hiểu biết của mình, cô chú trọng vào việc tạo thói quen và giúp học viên chú ý vào cách phát âm, ngữ điệu để nói tiếng Anh trôi chảy, hấp dẫn và hiệu quả.</p>
							</div>
							<div class="button-register-home">
								<a href="">ĐĂNG KÝ HỌC VỚI GIA SƯ NÀY</a>
							</div>
						</div>
					</div>
					<div class="clearfix"></div>
				</div>
			</div>
		</div>
	</div>

	<div class="container-fluid">

		<div class="student-say">
			<h2 class="h2Title">HỌC VIÊN NÓI VỀ ANTOREE</h2>
			<div class="comment-list">
				<div class="row">
					<div class="col-sm-5 teacher">
						<div class="avatar text-left">
							<img src="images/New-Layout/avatar1.png" class="float-left">
							<p class="name">Phạm Thị Hải Ninh</p>
							<p class="office">Phó tổng Giám đốc FLC</p>
							<p class="study-with">Học <strong>Professional English</strong> với gia sư <span class="green">Sagi</span></p>
							<div class="clearfix"></div>
						</div>
						<div class="text">
							<p>
								"Gia sư của tôi rất nhiệt tình, kiên trì hướng dẫn học viên. Tôi cảm thấy được hỗ trợ tốt và có thể nâng cao khả năng giao tiếp. Thường cuối tuần tôi khá bận và trong tuần cũng hay phải điều chỉnh lịch học nhưng gia sư vẫn linh động điều chỉnh theo lịch của tôi. Vì thế nên tôi đã đặt lịch học với giáo viên Sagi trong 2 năm."
							</p>
						</div>
					</div>
					<div class="col-sm-2"></div>

					<div class="col-sm-5 teacher margin-up">
						<div class="avatar text-left">
							<img src="images/New-Layout/avatar2.png" class="float-left">
							<p class="name">Nguyễn Bình Đức</p>
							<p class="office">Giám đốc Trung tâm thẻ Ngân hàng Việt Nga VRB</p>
							<p class="study-with">Học <strong>Giao tiếp</strong> với giáo viên <span class="green">Quỳnh Anh</span></p>
							<div class="clearfix"></div>
						</div>
						<div class="text">
							<p>
								“Công việc của mình bận, thường xuyên phải đi công tác nên hình thức học của Antoree rất phù hợp với mình. Ngoài ra việc học cũng theo yêu cầu của mình nên thường mình học giao tiếp để chuẩn bị cho các chuyến công tác nước ngoài. Giáo viên Quỳnh Anh thì vui tính và nhiệt tình. Giọng cô cũng hay và ngữ pháp cô nắm chắc nên cô củng cố giúp mình được cả phát âm và ngữ pháp. Cô cũng tâm lý và cách giảng dạy dễ hiểu.”
							</p>
						</div>
					</div>

					<div class="col-sm-5 teacher margin-down">
						<div class="avatar text-left">
							<img src="images/New-Layout/avatar3.png" class="float-left">
							<p class="name">Vũ Đức Dũng</p>
							<p class="office">Quản lý cấp cao và Nhà sáng lập của Nhóm Kinh doanh Google Việt Nam</p>
							<p class="study-with">Học <strong>giao tiếp</strong> với giáo viên <span class="green">Jonna</span></p>
							<div class="clearfix"></div>
						</div>
						<div class="text">
							<p>
								“Hình thức học của Antoree rất thú vị và hiệu quả. Đây là lần đầu tiên mình học với giáo viên bản ngữ mà lại được học trực tuyến. Vốn làm việc trong lĩnh vực công nghệ, mình thật sự rất ấn tượng với hình thức học tiên tiến này. Giáo viên của mình là cô Jonna giọng hay, dạy rất nhiệt tình và tận tâm. Phương pháp sư phạm của cô cũng tốt và học vui. Mình đã chia sẻ với cộng đồng fan của minh trên Facebook, Google và các bạn đã học cũng đều có phản hồi tốt về chất lượng giáo viên và dịch vụ của Antoree.”
							</p>
						</div>
					</div>
					<div class="col-sm-2"></div>

					<div class="col-sm-5 teacher ">
						<div class="avatar text-left">
							<img src="images/New-Layout/avatar4.png" class="float-left">
							<p class="name">Nguyễn Lệ Bích Thủy</p>
							<p class="office">Giáo viên Tiếng Anh tại trường THCS Đặng Công Bỉnh</p>
							<p class="study-with">Mẹ của học viên Nhật Minh</p>
							<div class="clearfix"></div>
						</div>
						<div class="text">
							<p>
								“Hôm nay, buổi học cuối khoá của con trai với cô giáo Pearl. Tâm trạng con trai không thể hiện, nhưng mẹ thì cảm xúc đong đầy. Mẹ và con trai cùng cám ơn cô giáo. Cuộc đời là chuỗi ngày gặp rồi xa nhau. Chúc nhau sự bình an và niềm vui. Về ngữ pháp tiếng Anh thì chị đã dạy hết cho con rồi, nhưng để luyện giao tiếp thì con cần được học với giáo viên nước ngoài. Ngay cả học sinh của chị, chị cũng khuyên chúng nó như vậy."
							</p>
						</div>
					</div>
					<div class="clearfix"></div>
				</div>
				
			</div>
		</div>
		<a href="" class="new-button trial-learn">ĐĂNG KÝ HỌC THỬ</a>
	</div>

	<div id="why">
		<div class="container">
			<h2 class="h2Title">
				TẠI SAO HỌC VIÊN CHỌN ANTOREE?
				<img src="images/New-Layout/heart-why.png" alt="" class="image-heart">
			</h2>
			<div class="list">
				<div class="child-of-list">
					<amp-img src="images/New-Layout/good-1.png" width="60" height="60"></amp-img>
					<h3 class="h3Title">HỌC 1 - 1 HIỆU QUẢ HƠN 40% SO VỚI LỚP ĐÔNG HỌC VIÊN</h3>
					<p>• Tăng 4 lần thời lượng giao tiếp với giáo viên</p>
					<p>• Lộ trình thiết kế riêng cho trình độ của từng người học</p>
					<p>• Không còn phải ngại ngùng khi nói. Chỉ có giáo viên lắng nghe và sửa từng lỗi cho bạn</p>
				</div>
				<div class="child-of-list">
					<amp-img src="images/New-Layout/good-2.png" width="60" height="60"></amp-img>
					<h3 class="h3Title">HỌC VỚI GIA SƯ BẢN NGỮ TĂNG VỐN TIẾNG ANH CỰC NHANH</h3>
					<p>• Giáo viên bản ngữ giúp bạn TIẾP THU Tiếng Anh MỘT CÁCH TỰ NHIÊN, khả năng ứng dụng thực tiễn cao, giảm thiểu các bài ngữ pháp khô cứng</p>
					<p>• Giáo viên bản ngữ giúp bạn phát âm chuẩn, tự tin khi giao tiếp với người nước ngoài.</p>
				</div>
				<div class="child-of-list">
					<amp-img src="images/New-Layout/good-3.png" width="60" height="60"></amp-img>
					<h3 class="h3Title">HỌC ONLINE LỰA CHỌN ĐA DẠNG, TIẾT KIỆM THỜI GIAN KHÔNG CẦN DI CHUYỂN</h3>
					<p>• Bạn có thể học bất cứ khóa học nào: Tiếng Anh giao tiếp, luyện thi chứng chỉ tiếng Anh,học tiếng anh chuyên ngành kế toán lập trình, tiếng anh thương mại, tiếng anh phỏng vấn xin việc, tiếng anh du học, tiếng Anh du lịch, tiếng Anh phỏng vấn định cư...</p>
					<p>• Bạn được lựa chọn từ hơn 1000+ gia sư bản ngữ Mỹ, Anh, Úc, Philippines</p>
				</div>
				<div class="child-of-list">
					<amp-img src="images/New-Layout/good-4.png" width="60" height="60"></amp-img>
					<h3 class="h3Title">HỌC PHÍ HỢP LÝ</h3>
					<p>• Được học với giảng viên bản ngữ chi phí phù hợp với khả năng chi trả của hầu hết học viên.</p>
					<p>• Tiết kiệm đến 40% so với học ở các trung tâm, mô hình khác (Học phí cả khóa của Antoree từ 2.7 triệu cho 30 buổi. Học phí trung bình ở các trung tâm lớp đông người là 80 - 200k/buổi tương đương 4 triệu cho khóa 20 buổi với lớp học quy mô trên 6 học viên).</p>
				</div>
				<div class="child-of-list">
					<amp-img src="images/New-Layout/good-5.png" width="60" height="60"></amp-img>
					<h3 class="h3Title">MIỄN PHÍ 100% TÀI LIỆU</h3>
					<p>Không cần phải mua tài liệu đắt tiền, bạn được gia sư chuẩn bị toàn bộ giáo trình online và MIỄN PHÍ.</p>
				</div>
				<div class="child-of-list">
					<amp-img src="images/New-Layout/good-6.png" width="60" height="60"></amp-img>
					<h3 class="h3Title">MIẾN PHÍ HỌC THỬ</h3>
					<p>Bạn được học thử không tính phí 01 buổi với giáo viên bản ngữ.</p>
				</div>
				<div class="child-of-list">
					<amp-img src="images/New-Layout/good-7.png" width="60" height="60"></amp-img>
					<h3 class="h3Title">HỖ TRỢ 24/7</h3>
					<p>• Bạn được đổi giáo viên nếu giáo viên không phù hợp.</p>
					<p>• Bạn được đổi/ hoãn lịch học và hỗ trợ kỹ thuật trong suốt quá trình học.</p>
				</div>
			</div>
			<div class="button-register">
				<a href="" class="new-button trial-learn">KIỂM TRA TRÌNH ĐỘ MIỄN PHÍ</a>
			</div>
		</div>
	</div>

	<div id="media" class="bg-gray">
		<div class="container">
			<h2 class="h2Title">BÁO CHÍ NÓI VỀ ANTOREE</h2>
			<div class="table text-center">
				<a href="http://ictnews.vn/khoi-nghiep/antoree-san-giao-dich-hoc-tieng-anh-dau-tien-cho-nguoi-viet-121863.ict" target="_blank">
					<amp-img src="images/New-Layout/img_ict_new.png" width="180" height="55"></amp-img>
				</a>
				<a href="http://ndh.vn/co-gai-8x-bo-luong-3-000-usd-thang-de-khoi-nghiep-voi-mo-hinh-gia-su-kieu-uber--20160304085045463p128c143.news" target="_blank">
					<amp-img src="images/New-Layout/img_ndh.png" width="166" height="55"></amp-img>
				</a>
				<a href="http://thanhnien.vn/gioi-tre/co-gai-tre-khoi-nghiep-bang-du-an-uber-tim-gia-su-676801.html" target="_blank">
					<amp-img src="images/New-Layout/thanhnien_1.png" width="190" height="40"></amp-img>
				</a>
				<a href="http://doisong.vn/tu-bo-cong-viec-trieu-do-nu-doanh-nhan-8x-thanh-cong-khi-mang-mo-hinh-gia-su-uber-den-voi-ngan-nguoi-n38311.html" target="_blank">
					<amp-img src="images/New-Layout/logo-v3.png" width="182" height="54"></amp-img>
				</a>
				<a href="http://vietnamnet.vn/vn/kinh-doanh/doanh-nhan/bo-giam-doc-1-trieu-usd-o-singapore-lam-uber-gia-su-thu-4-ty-thang-333618.html" target="_blank">
					<amp-img src="images/New-Layout/logo-vnn.svg" width="130" height="50"></amp-img>
				</a>
				<a href="http://thanhnien.vn/gioi-tre/co-gai-tre-khoi-nghiep-bang-du-an-uber-tim-gia-su-676801.html" target="_blank">
					<amp-img src="images/New-Layout/img_tech_asia.png" width="229" height="40"></amp-img>
				</a>
				<a href="http://thieunien.vn/34-bi-kip/14218-hoc-tieng-anh-phuong-phap-nao-moi-dung.html" target="_blank">
					<amp-img src="images/New-Layout/hoctro.png" width="171" height="50"></amp-img>
				</a>
			</div>
		</div>
	</div>

	<div id="clients">
		<div class="container-fluid">
			<h2 class="h2Title">KHÁCH HÀNG VÀ ĐỐI TÁC CỦA ANTOREE</h2>
			<div class="table text-center">
				<a href="">
					<amp-img src="images/New-Layout/customer-client-1.png" width="150" height="150"></amp-img>
				</a>
				<a href="">
					<amp-img src="images/New-Layout/customer-client-2.png" width="198" height="150"></amp-img>
				</a>
				<a href="">
					<amp-img src="images/New-Layout/customer-client-3.png" width="300" height="150"></amp-img>
				</a>
				<a href="">
					<amp-img src="images/New-Layout/customer-client-4.png" width="200" height="150"></amp-img>
				</a>
				<a href="">
					<amp-img src="images/New-Layout/customer-client-5.png" width="200" height="150"></amp-img>
				</a>
				<a href="">
					<amp-img src="images/New-Layout/customer-client-6.png" width="200" height="150"></amp-img>
				</a>
				<a href="">
					<amp-img src="images/New-Layout/customer-client-7.png" width="300" height="150"></amp-img>
				</a>
			</div>
		</div>
	</div>

	<div id="form-register">
		<!-- Form register -->
		<div class="form">
			<h2 class="h4Title">ĐĂNG KÝ HỌC THỬ MIỄN PHÍ!</h2>
			<form name="frmRegister" novalidate>
				<p>Bạn được tặng 1 buổi học thử miễn phí với Giảng viên Nước ngoài !</p>
				<div class="form-group">
					<input name="name" type="text" class="form-control"  id="exampleInputText" placeholder="Tên của bạn" required />
				</div>
				<div class="form-group">
					<input name="phone" type="text" class="form-control" id="exampleInputSDT" placeholder="Số điện thoại" required />
				</div>
				<div class="form-group">
					<input name="email" type="text" class="form-control" id="exampleInputEmail" placeholder="Địa chỉ email" required />
				</div>
				<p>
					Nhận ngay ebook dạy phát âm giọng Mỹ chuẩn trị giá 100.000đ khi hoàn tất đăng ký.
				</p>
				<div class="btnSubmit">
					<button class="bz-btn btn-default bg-onrange register" type="submit">
						<span class="content">Đăng ký</span>
						<span class="prosse"></span>
					</button>
				</div>
			</form>
		</div>
		<!-- End: Form register -->
	</div>

</div>